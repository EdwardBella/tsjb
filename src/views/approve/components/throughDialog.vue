<template>
	<div>
		<el-dialog append-to-body v-dialogDrags title="通过" width="640px" :visible="visible" @close="handleClose">
			<el-form :model="form" :rules="rules" ref="form" label-width="140px" class="white-card">
				<el-form-item label="通过备注:" prop="comment">
	...
	export default {
		...
		methods: {
			show() {
				...
				//   });
				//
				templateApi.detail(1027)
					.then(res => {
						this.form.comment = res.result.value;
					});
			},
			handleSubmit() {
				this.validateAndConfirm("form", "是否确认通过？")
					.then(() => {
						this.submitting = true;
						return workOrderApi.audit.success({
							...this.form,
							id: this.id,
							operation: this.type
						});
