		...
		<div slot="content" style="position: relative;">
			<div class="contents">
				<div style="width: 65%;display: flex;flex-direction: column;">
					<tabs :tabsList="tabsList" tabOrigin="结案类型" widthValue="320px" />
				...
				<div style="width: 35%;">
					...
					<vueSeamlessScroll :data="dataLists" :class-option="seamlessScrollOptions"
						style="height: 300px; overflow: hidden" class="auto-scroll-table">
						<ul>
							<li v-for="(item, index) in dataLists" :key="index" class="li-item">
								<span class="ellipsis" style="flex: 3;">{{ item.name }}</span>
								<span style="flex: 1;font-size: 18px !important;color: #2C65DA;font-weight: 900;">{{ item.count }}</span>
							</li>
						</ul>
					</vueSeamlessScroll>
				</div>
