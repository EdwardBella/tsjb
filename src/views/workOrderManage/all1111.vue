			...
			<div class="search-content" :style="{height:heightSize}">
				<el-form :model="queryParams" :inline="true" label-position="left" ref="queryForm" label-width="110px">
					...
					<el-form-item label="满意度:" prop="satisfiedScore">
						<el-select ref="satisfiedScore" v-model="queryParams.satisfiedScore" multiple collapse-tags
							clearable placeholder="默认查询全部" style="width: 260px;" @change="statusChanged">
				...
				<el-table v-loading="tableData.loading" element-loading-text="拼命加载中" border stripe style="width: 100%"
					...
					<el-table-column label="案件名称" show-overflow-tooltip prop="title" min-width="500px"
						...
						<template slot-scope="scope">
							<span v-if="scope.row.historyTrack == '1'" class="chong"
							...
							<img style="width: 20px;"
								...
								:src="filterTimer(scope.row.currentStepDeadline,scope.row.status,2)" alt="" />
							<span
					...
					<el-table-column label="阶段时限" prop="age" width="110px" header-align="left" align="left">
						<template slot-scope="scope">
							<span
								v-if="timeDifference('',scope.row.currentStepDeadline).days != 0 || timeDifference('',scope.row.currentStepDeadline).hours != 0"
	...
	export default {
		...
		methods: {
			...
			getList() {
				...
				let keys = ['workOrderNumber', 'companyName', 'createUsername', 'mobile', 'itemSource', 'addressId',
				...
				for (let i = 0; i < keys.length; i++) {
					if (this.queryParams[keys[i]] != '' || this.queryParams[keys[i]].length > 0) {
				...
				params.allStatus = this.queryParams.allStatus

				console.log(params, "&&&&&&&&&&&&&&&&&&&&&&")
				this.tableData.loading = true
				workOrderApi.distribute.list(params).then(res => {
					console.log(res, ']]]]]]]]]]]]]]]]]')
