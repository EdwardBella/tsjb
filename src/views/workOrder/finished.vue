	...
	<div class="dbContent">
		...
		<div>
			<div class="search-content" :style="{height:heightSize}">
				<el-form :model="queryParams" :inline="true" label-position="left" ref="queryForm" label-width="100px">
					...
					<el-form-item label="" prop="checkVisit">
						<el-checkbox true-label="1" false-label="0" v-model="queryParams.checkVisit"
							@change="handleQuery">回访核实</el-checkbox>
					</el-form-item>
			...
			<div class="table-cn">
				<el-table v-loading="tableData.loading" element-loading-text="拼命加载中" border stripe style="width: 100%"
					...
					<el-table-column label="办结时间" prop="finishTime" width="170px" header-align="left" align="left" />
		...
		<el-drawer title="我是标题" size="85%" :visible.sync="drawer" :with-header="false" destroy-on-close>
			<caseDetails @closeDrawer="closeDetail" :workOrderId="workOrderId" :workOrderNo="workOrderNo"
				basicType='finish' @success="getList" />
		</el-drawer>
	</div>
</template>
	...
	export default {
		...
		data() {
			return {
				...
				statusArr: ajztStatusDict3, //案件状态
				areas: [],
				deptArr: [],
				heightSize: 'auto',
				drawer: false,
		...
		methods: {
			...
			getList() {
				...
				let keys = ['workOrderNumber', 'title', 'complainAddressId', 'processDepartmentCode', 'checkVisit',
					'timeForProcess'
				]
				...
				workOrderApi.finished.list(params).then(res => {
					console.log(res, ']]]]]]]]]]]]]]]]]')
